...
public:
	...
	friend void Swap(KDeque<T, grow_const1> &op1, KDeque<T, grow_const2> &op2);
...
template <class T>
class Iterator {
	const KDeque<T>* collection;
	KIterator<T> pos;
public:
	Iterator(const KDeque<T>& collection) :collection(&collection), pos(collection.begin()) {}
	Iterator(const KDeque<T>& collection, const KIterator<T>& it) :collection(&collection), pos(it) {}

	T & current() {
		if (pos == collection->end())
			throw std::out_of_range("current element does not excist");
		return *pos;
	}

	Iterator& next() {
		if (pos == collection->end())
			throw std::out_of_range("next element does not excist");
		++pos;
		return *this;
	}

	bool has_next() {
		if (pos == collection->end()|| ++pos == collection->end()) {
			pos--;
			return false;
		}
		pos--;
